...
public class Helper {
    ...
    public int getPossibleAttacks (ArrayList<Piece> board, int index) {
        ...
        if (board.size() < index)
            return 0;
        if (board.get(index).color == RED)
            targetColor = BLUE;
        else
            targetColor = RED;
        if (board.get(index).color == RED || board.get(index).isKing()) {
            if (!this.checkSquare(board.get(index).x + 2,
                    board.get(index).y + 2, board) && !this.checkSquare(board.get(index).x + 1,
                    board.get(index).y + 1, board, targetColor) && !this.checkSquare(board.get(index).x - 1,
                    board.get(index).y - 1, board, targetColor) && !this.checkSquare(board.get(index).x + 1,
    ...
    public int getPossibleBenefitalAttacks (ArrayList<Piece> board, int index) {
        ...
        if (board.get(index).color == RED || board.get(index).isKing()) {
            if (!this.checkSquare(board.get(index).x + 2,
                    board.get(index).y + 2, board) && !this.checkSquare(board.get(index).x + 1,
                    board.get(index).y + 1, board) && !this.checkSquare(board.get(index).x + 3, board.get(index).y + 3, board, targetColor)) {
                attacks++;
            }
